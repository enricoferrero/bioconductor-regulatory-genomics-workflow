...
knitr::opts_chunk$set(message = FALSE, warning = FALSE, fig.width = 5, fig.height = 5)
```
...
While many of these associations might not actually be relevant for the disease aetiology [@Boyle2017], these methods hold much promise to guide pharmaceutical scientists towards the next generation of drug targets [@Finan2017]. 

Arguably, one of the biggest challenges in translating findings from GWASs to therapies is that the great majority of single nucleotide polymorphisms (SNPs) associated with disease are found in non-coding regions of the genome, and therefore cannot be easily linked to a target gene [@Maurano2012].
...
#biocLite(c("DESeq2", "GenomicFeatures", "GenomicInteractions", GenomicRanges", "ggplot2", "Gviz", gwascat", "recount", "pheatmap", "RColorBrewer", "rtracklayer", "R.utils", "splitstackshape", "VariantAnnotation"))
```
...
```{r figure1, fig.cap = 'Clustered heatmap showing distances between samples.'}
library(pheatmap)
...
```{r figure2, fig.cap = 'Principal component analysis with samples coloured according to their disease status.'}
...
```{r figure3, fig.cap = 'MA plot showing genes differentially expressed in SLE patients compared to healthy patients.'}
...
Note that p-values lower than 1e-25 are truncated in the figure and that we have to load `ggplot2` [@Wickham2009] to modify the look of the plot:

```{r figure4, fig.cap = 'Manhattan plot showing variants significantly associated with SLE.', fig.height = 7, fig.width = 6}
library(ggplot2)
...
As this LD expansion step can greatly increase the number of variants under investigation, we will skip it and refer the reader to the Ensembl REST API [@Yates2015], the [Ensembl Linkage Disequilibrium Calculator](http://www.ensembl.org/Homo_sapiens/Tools/LD?db=core) and the Bioconductor packages `trio` [@Schwender2015] and `ldblock` [@Carey2017c] to perform this task.
...
We can visualise where these SNPs are located with `ggplot2` [@Wickham2009] (Figure 5).

```{r figure5, fig.cap = 'Barplot showing genomic locations associated with SLE variants.'}
...
The [`R.utils`](https://cran.r-project.org/package=R.utils) package is required to extract the gzipped file: 
...
We note that gene IDs in GTEx are mapped to GENCODE v19 [@GTEx2017], while we are using the newer v25 for the DEGs.
...
These regions were then mapped to enhancers based on the Ensembl Regulatory Build [@Zerbino2015] and can be accessed in the supplementary data of the paper:
...
rownames(prioritised_hits) <- NULL
prioritised_hits
```

There are at least three good options to visualise genomic data and interactions in Bioconductor: ggbio [@Yin2012], Sushi [@Phanstiel2014] and Gviz [@Hahne2016] coupled with the GenomicInteractions package [@Harmston2015].
