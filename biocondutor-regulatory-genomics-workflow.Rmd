...
While many of these associations might not actually be relevant for the disease etiology [@Boyle2017], these methods hold much promise to guide towards the next generation of drug targets [@Finan2017]. 
...
Over the last few years, the GTEx consortium assembled a valuable resource by performing large-scale mapping of genome-wide correlations between genetic variants and gene expression across 44 human tissues [@Gtex2017].
...
Similarly, the FANTOM5 consortium used cap analysis of gene expression (CAGE) to identify promoters and enhancers across hundreds of cells and tissues [@Fantom2014].
...
To address this, we developed STOPGAP (systematic target opportunity assessement by genetic association predictions), a database of disease variants mapped to their most likely target gene(s) using different types of regulatory genomic data [@Shen2017].
The database is currently undergoing a major overhaul and will eventually be superseeded by POSTGAP [@Ensembl2017].
...
We will use eQTL data from GTEx [@Gtex2017], FANTOM5 correlations between promoters and enhancers [@Andersson2014] and promoter capture Hi-C data [Javierre2016].
...
#biocLite(c("DESeq2", "GenomicFeatures", "GenomicRanges", "ggplot2", "gwascat", "ldblock", recount", "pheatmap", "RcolorBrewer", "rtracklayer", "R.utils", "splitstackshape", VariantAnnotation"))
```
...
Other Bioconductor packages that can be used to get data from gene expression experiments directly in R are `GEOquery` [@Davis2007] and `ArrayExpress` [@Kauffmann2009]. 
...
We will use the `pheatmap` [@Kolde2015] and `RColorBrewer` [Neuwirth2014] packages for drawing the heatmap:
...
We can also visualise the log fold changes using an MA plot:
...
An alternative is to use the GRASP [@Eicher2017] database with the `grasp2db` [@Carey2017b] package.
...
Genotyping arrays typically include a very small fraction of all possible SNPs in the human genome, and there is no guarantee that the SNPs on the array are the true casual SNPs [@Bush2012].
The alleles of other SNPs can be imputed thanks to the structure of linkage disequilibrium (LD) blocks present in chromosomes. 
When linking variants to target genes, it is therefore important to perform LD expansion and take into consideration neighbouring SNPs that are inherited with the tag SNPs.
We can use the `ldblock` Bioconductor package [@Carey2017c] for this task:
...
We can visualise where these SNPs are located with `ggplot2` [@Wickham2009]:
...
As expected, the great majority of SNPs is located within introns and in intergenic regions [@Maurano2012].
...
The FANTOM5 consortium profiled gene expression across a large panel of tissues and cell types using CAGE [@Fantom2014; Andersson2014].
...
From the publication [@Andersson2014] we know these are mapped to genome reference GRCh37, so we will uplift them to GRCh38 using `rtracklayer` [@Lawrence2009] and a mapping ("chain") file.
The `R.utils` package [@Bengtsson2014] is required to extract the gzipped file: 
...
We will use the `GenomicRanges` package [@Larence2013] to compute the overlap between SNPs and enhancers:
...
These regions were then mapped to enhancer based on the Ensembl Regulatory Build [@Zerbuno2015] and can be accessed in the supplementary data of the paper:
